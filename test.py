...
class TestPatientKFold(unittest.TestCase):
    def test_simple_split(self):
        ...
        for fold, (train_patients, test_patients) in enumerate(p):
            self.assertEqual(train_patients, results[fold][0], "Fail while looping")
            self.assertEqual(test_patients, results[fold][1], "Fail while looping")
    ...
    def test_dataframe_split(self):
        ...
        results = [
            ...
            (
                ...
                {"patient": [5, 5, 5], "other_columns": ["f", "g", "h"]},
            ),
            # fold 4
            (
                {
        ...
        for fold, (train_patients, test_patients) in enumerate(p):
            self.assertEqual(
                list(train_patients["patient"]),
                results[fold][0]["patient"],
                "Fail while looping",
            )
            self.assertEqual(
                list(train_patients["other_columns"]),
                results[fold][0]["other_columns"],
                "Fail while looping",
            )

            self.assertEqual(
                list(test_patients["patient"]),
                results[fold][1]["patient"],
                "Fail while looping",
            )
            self.assertEqual(
                list(test_patients["other_columns"]),
                results[fold][1]["other_columns"],
                "Fail while looping",
            )
        ...
        self.assertEqual(
            list(train_patients["patient"]),
            result_train["patient"],
            "Fail while looping",
        )
        self.assertEqual(
            list(train_patients["other_columns"]),
            result_train["other_columns"],
            "Fail while looping",
        )

        self.assertEqual(
            list(test_patients["patient"]), result_test["patient"], "Fail while looping"
        )
        self.assertEqual(
            list(test_patients["other_columns"]),
            result_test["other_columns"],
            "Fail while looping",
        )
